...
def search_notes():
    ...
    try:
        ...
        sql = f"SELECT * FROM notes WHERE (title LIKE '%{query}%' OR content LIKE '%{query}%') AND user_id in (SELECT id FROM users WHERE username = '{session['user']}')"
...
def delete_note(note_id):
    ...
    try:
        ...
        # Ensure the user is only allowed to delete their own notes
