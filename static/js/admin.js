// Function to validate username (only alphanumeric and underscores, 3-30 chars)
function isValidUsername(username) {
    return /^[a-zA-Z0-9_]{3,30}$/.test(username) && /[a-zA-Z]/.test(username);
}

// Function to validate passwords (at least 8 characters, letters + numbers + special symbols)
...
function initializeApp() {
    ...
    async function handleAddUser() {
        ...
        if (!isValidUsername(username)) {
            alert("Invalid username. Username should be (3-30 chars) with atleast one letter. Only letters, numbers, and underscores are allowed.");
            return;
        }

        if (!isValidPassword(password)) {
            alert("Password must be 8+ chars with atleast 1 uppercase, 1 lowercase letter, 1 number and 1 special character.");
            return;
        }
